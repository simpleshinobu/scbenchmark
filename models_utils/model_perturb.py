...
class scModel(nn.Module):
    ...
    def forward(self,src: Tensor,values: Tensor,src_key_padding_mask: Tensor, base_emb=None, perturb_trans=None, perturb_cls=None):
        output = {}
        gene_emb = self.embedding(src)
        if base_emb is not None:
            gene_emb[:,0:1] += base_emb.weight.T.unsqueeze(0)
        if perturb_trans is not None:
            perturb_emb = self.embedding(perturb_cls)
            perturb_emb = perturb_trans[0](perturb_emb) + perturb_trans[1].weight.T
            gene_emb[:,0:1] += perturb_emb.unsqueeze(1)
        if self.dict_size != self.args.embsize:
