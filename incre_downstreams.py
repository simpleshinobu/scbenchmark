...
class ExpressionDataset(Dataset):
    def __init__(self, data, max_length, pad_token_id, pad_value, args = None, filter = ""):
        self.data = data
        ...
        if filter in ["adamson", "norman", "dixit"]:
            self.class_name = "single_ctrl"
        else:
            self.class_name = "cls_name"
        self.class_label_map = {label: idx for idx, label in enumerate(set(self.data[self.class_name]))}
        self.tmp_vocab_idx = None
    def __len__(self):
        return len(self.data[self.class_name])
    ...
    def __getitem__(self, index):
        ...
        cls_label = self.data[self.class_name][index]
...
def extract_features(model, data_loader, vocab, args):
    ...
    with torch.no_grad():
        for data_dict in data_loader:
            ...
            embeddings.append(cell_embeddings.cpu().numpy())  # Transfer to CPU and convert to numpy
...
def run(args, vocab, dataset,model_name):
    ...
    model_path = Path(args.model_dir) / f"{model_name}.pt"
    ...
    model = model.cuda()
    ##################knn accu
    if args.eval_knn or args.only_knn:
    ...
    for epoch in range(1,args.num_epochs+1):
        ...
        if epoch % args.eval_epoch == 0:
            # print("Evaluting for epoch {}".format(epoch))
            accuracy = evaluate(model, classifier, eval_loader, vocab, args)
            # print("Evaluting for epoch {}".format(epoch),f'Accuracy on evaluation data: {accuracy:.2f}%')
            if accuracy > best_accuracy:
                best_accuracy = accuracy
                best_epoch = epoch
                # print(f'New best accuracy: {best_accuracy:.2f}% at Epoch {best_epoch}')
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f'{current_time}:Linear Accuracy on evaluation data: ,{best_accuracy:.2f}% best epoch{best_epoch}')
    return {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-emb-style", type=str, choices=["continuous"], default="continuous",
    ...
    parser.add_argument("--only_knn", action="store_true")
    parser.add_argument("--add_note", type=str, default="normal")
    parser.add_argument("--skip_stage1",action="store_true")
    parser.add_argument("--filter_names", nargs='+',
                        default  = ["pancread" , "dengue" , "scanorama","lupus","Myeloid", "adamson" ],
                        help="List of filter names to process.")
    args = parser.parse_args()
    print("selected datssets", args.filter_names)
    args.mask_value = -1
    args.pad_value = 0
    args.cls_value = 0
    ...
    args.fp16 = True
    set_seed(args.seed)
    stage_tokens_path = Path(args.model_dir) / "stage_tokens_indices.pt"
    stage_tokens_indices_origin = torch.load(stage_tokens_path)
    stage_tokens_indices_testing = [] ##in 11.02
    for k in range(len(stage_tokens_indices_origin)):
        stage_tokens_indices_testing.append(list(set(sum(stage_tokens_indices_origin[:k+1], [])[:])))
    print("Stage tokens indices loaded successfully.")
    model_dir = Path(args.model_dir)
    model_files = sorted(model_dir.glob("model-stage*.pt"))
    combined_results = pd.DataFrame()
    for k, model_file in enumerate(model_files):
        model_name = model_file.stem  # Extract the model name (e.g., 'model-stage0')
        # Prepare data for DataFrame
        if k == 0:
            continue
        if args.skip_stage1 and k == 1:
            continue
        results = []
        print("current_size", len(stage_tokens_indices_testing[k]))
        for filter_name in args.filter_names:
            print(f"Processing filter: {filter_name}, model: {model_name}")
            all_accuracies = []
            all_knn_accuracies = []
            for trial in range(args.num_trials):
                ...
                input_file = input_path / f"{filter_name}_data.pt"
                data_saved_all = torch.load(input_file)
                dataset = ExpressionDataset(data_saved_all, args.test_maxseq, vocab["<pad>"], args.pad_value, args=args, filter=filter_name)
                temp_token_idx = torch.Tensor(stage_tokens_indices_testing[k]).long()
                temp_token_idx = torch.sort(temp_token_idx, descending=False)[0]
                dataset.tmp_vocab_idx = temp_token_idx
                results_dict = run(args, vocab, dataset, model_name)
            ...
            results.append([filter_name, mean_accuracy, std_accuracy, mean_knn_accuracy, std_knn_accuracy])

            print(
                f'Dataset: {filter_name}. Average KNN Accuracy/ cls Accuracy across trials: {mean_knn_accuracy:.3f}±{std_knn_accuracy:.3f}/{mean_accuracy:.3f}±{std_accuracy:.3f}')
            print("#" * 80, "\n", "#" * 80)

        results_df = pd.DataFrame(results, columns=["Filter Name", "Mean Accuracy", "Std Accuracy", "Mean KNN Accuracy",
                                                    "Std KNN Accuracy"])
        transposed_df = results_df.T
        save_dir = Path(f'./save_result_incre/{model_dir.name}')
        save_dir.mkdir(parents=True, exist_ok=True)
        results_file = save_dir / f'results_{model_name}.xlsx'
        excel_writer = pd.ExcelWriter(results_file, engine='openpyxl')
        transposed_df.to_excel(excel_writer, index=False)
        excel_writer.close()
        print(f'Results saved to Excel successfully in {results_file}.')
        combined_results = pd.concat([combined_results, transposed_df])
    final_results_file = save_dir / f'combined_results_{args.add_note}.xlsx'
    combined_results.to_excel(final_results_file, index=False)
    print(f"All results have been successfully merged and saved to {final_results_file}")